...
__global__ void blurKernel(char* Pin, char* Pout, int W, int H){
...
void blurGPU(char* Pin_h, char* Pout_h, int W, int H){
    ...
    blurKernel<<<gridSize, blockSize>>>(Pin_d, Pout_d, W, H);
...
int main(int argc, char** argv){
    ...
    startTime(&timer);
    blurGPU(Pin, Pout, W, H);
    stopTime(&timer);

    cudaDeviceSynchronize();

    printElapsedTime(timer, "blurGPU", GREEN);
